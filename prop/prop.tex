...
\end{eqnarray}
%% Discretely, let $u_i^{[k],n}$ denote the numerical solution on
%% subdomain $\Omega_i$, waveform iterate $k$, and time $t^n$.  Then, a
%% backward Euler differentiation of
%% equation~(\ref{ongbw_mini_13_eqn:pde3}) gives
%% \begin{eqnarray*}
%%   \frac{u_i^{[k],n+1} - u_i^{[k],n}}{\Delta t} = \mathcal{L}(t^{n+1},u_i^{[k],n+1}),\\
%% \end{eqnarray*}

%% \begin{algorithm}
%% \caption{The classic waveform relaxation algorithm allows for each
%%   subdomain to independently advance the solution on their respective
%%   subdomains using time integrators and time steps of their choosing.
%%   (i.e. each domain can use different time integrators and different
%%   time steps).  During each waveform iteration, boundary data on each
%%   subdomain is aggregated for the entire compuational time interval
%%   before boundary data is exchanged between neighboring subdomains.}
%% \label{ongbw_mini_13_alg1}
\begin{table}[htbp]
\noindent\hrulefill
...
%% 2.  for each domain (MPI Parallelization)
%% 3.     Guess $u_i^{[0]}(z,t), \quad z \in \partial\Omega_i\cup\partial\Omega_j$
%% 6.  end 
%% 7.  for each waveform iteration 
%% 8.     for each subdomain (MPI Parallelization)
%% 9.        for each time step
%% 10.          advance solution on subdomain 
%% 11.        end
%% 12.     end
%% 13.     MPI Comm (exchange boundary data & check convergence)
%% 14. end
%% 15. MPI Finalize
%% \end{verbatim}
.\hrulefill  \\
